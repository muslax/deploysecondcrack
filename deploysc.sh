...
cd ~/src ; wget http://developer.axis.com/download/distribution/apps-sys-utils-start-stop-daemon-IR1_9_18-2.tar.gz;
...
cd ~; sudo cp ~/src/deploysecondcrack/config-files/dropbox-service /etc/init.d/dropbox;
sudo cp ~/src/deploysecondcrack/config-files/sysconfig-dropbox /etc/sysconfig/dropbox;
...
sudo crontab ~/src/deploysecondcrack/config-files/crontab.example;
...
sudo cp ~/src/deploysecondcrack/config-files/main.css ~/Dropbox/Blog/assets/css/main.css;
...
sudo chmod -R o+x  ~/Dropbox;
sudo cp ~/src/deploysecondcrack/config-files/config.php.example ~/secondcrack/config.php;
